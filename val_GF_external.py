...
parser.add_argument(
    "--dataset_name", default="FLARE23", type=str, help="3Dircadb1, FLARE23, Rider")


parser.add_argument(
    "--test_data_path", default="./data/external/", type=str, help="test_data_path")
parser.add_argument(
    "--save_prediction_path", default="./pred/external_GF/", type=str, help="test_prediction_path")
parser.add_argument(
...
def get_test_loader(args):
    ...
    """
    if args.dataset_name == 'Rider':
        a_min, a_max = -900.0, 650.0
    else:
        a_min, a_max = -175.0, 250.0
    ...
    data_path = os.path.join(args.test_data_path, 'imagesTs')
    label_path = os.path.join(args.test_data_path, 'labelsTs')
    ...
    for item in dataset_list:
        name = item
        print(name)
        test_img_path = os.path.join(data_path, name)
        test_label_path = os.path.join(label_path, name[:-12] + '.nii.gz')  # [:-12] +'.nii.gz'
...
def main():
    ...
    args.test_data_path = args.test_data_path + args.dataset_name
    args.save_prediction_path = args.save_prediction_path + args.dataset_name

    log_path = os.path.join(args.save_prediction_path, str(args.dataset_name) + "_log.txt")
    ...
    with torch.no_grad():
        for idx, batch_data in enumerate(test_loader):
            ...
            os.rename(os.path.join(args.save_prediction_path, name[:-7]+'_trans.nii.gz'), os.path.join(args.save_prediction_path, name[:-12]+'.nii.gz'))
