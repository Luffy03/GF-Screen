...
class ResNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # x = self.avgpool(x)
        x = F.adaptive_max_pool3d(x, (1, 1, 1)).view(x.size()[0], -1)
...
if __name__ == '__main__':
    # model = resnet10(pretrained=True, n_input_channels=1, num_classes=2, feed_forward=False, shortcut_type='B', bias_downsample=False)
    model = resnet18(pretrained=False, n_input_channels=1, num_classes=2, feed_forward=False,
                                     shortcut_type='A', bias_downsample=True)

    model = resnet34(pretrained=False, n_input_channels=1, num_classes=2, feed_forward=False,
                     shortcut_type='A', bias_downsample=True)

    pytorch_total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print("Glance Total parameters count", pytorch_total_params)
